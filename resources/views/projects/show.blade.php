            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="bg-white p-4 sm:p-6 rounded-lg shadow">
                    ...
                    <div>
                        ...
                        <div x-show="activeTab === 'info'" x-cloak><div class="grid grid-cols-1 md:grid-cols-2 gap-8"><div class="space-y-6"><div class="bg-gray-50 p-4 rounded-lg"><h3 class="text-lg font-semibold mb-2 text-gray-800">Distribusi Status Tugas</h3><canvas id="taskStatusChart"></canvas></div><div class="bg-gray-50 p-4 rounded-lg"><h3 class="text-lg font-semibold mb-2 text-gray-800">Tim Proyek</h3><ul><li class="flex items-center space-x-2"><span class="font-bold text-gray-700">Ketua Tim:</span><span>{{ optional($project->leader)->name ?? 'N/A' }}</span></li></ul><h4 class="font-semibold mt-4 text-gray-800">Anggota:</h4><ul class="list-disc list-inside mt-2 text-gray-700">@foreach($project->members as $member)<li>{{ $member->name }}</li>@endforeach</ul></div></div><div class="space-y-6"><div class="bg-gray-50 p-4 rounded-lg"><h3 class="text-lg font-semibold mb-2 text-gray-800">Detail Proyek</h3><p class="text-gray-700">{{ $project->description }}</p><div class="text-sm mt-4 grid grid-cols-2 gap-2 text-gray-600"><p>Dibuat Oleh:</p><p class="font-semibold text-gray-800">{{ $project->owner->name ?? 'N/A' }}</p><p>Tanggal Mulai:</p><p class="font-semibold text-gray-800">{{ $project->start_date ? \Carbon\Carbon::parse($project->start_date)->format('d M Y') : '-' }}</p><p>Tanggal Selesai:</p><p class="font-semibold text-gray-800">{{ $project->end_date ? \Carbon\Carbon::parse($project->end_date)->format('d M Y') : '-' }}</p></div></div><div class="bg-gray-50 p-4 rounded-lg"><h3 class="text-lg font-semibold mb-2 text-gray-800">Aktivitas Terbaru</h3><ul class="space-y-3">@foreach($project->activities->take(5) as $activity)<li class="text-sm text-gray-600 border-b pb-2"><span class="font-semibold text-gray-800">{{ optional($activity->user)->name ?? 'User Telah Dihapus' }}</span> @switch($activity->description)@case('created_project')membuat proyek ini @break @case('updated_project')memperbarui proyek ini @break @case('created_task')membuat tugas "{{ optional($activity->subject)->title ?? '...' }}" @break @case('updated_task')memperbarui tugas "{{ optional($activity->subject)->title ?? '...' }}" @break @case('deleted_task')menghapus sebuah tugas @break @default melakukan sebuah aktivitas @endswitch<span class="block text-xs text-gray-400">{{ $activity->created_at->diffForHumans() }}</span></li>@endforeach</ul></div></div></div></div>
