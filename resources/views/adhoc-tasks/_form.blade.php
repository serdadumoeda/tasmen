...
<div class="space-y-6"> {{-- Mengubah space-y-4 menjadi space-y-6 untuk konsistensi dengan tampilan kartu --}}
    <div>
        ...
        <input type="text" name="title" id="title" class="block mt-1 w-full rounded-lg shadow-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 transition duration-150" value="{{ old('title') }}" required> {{-- Mengubah rounded-md menjadi rounded-lg, menambahkan fokus, dan transisi --}}
    </div>
    ...
    <div>
        ...
        <textarea name="description" id="description" rows="4" class="block mt-1 w-full rounded-lg shadow-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 transition duration-150" rows="3">{{ old('description') }}</textarea> {{-- Mengubah rounded-md menjadi rounded-lg, menambahkan fokus, dan transisi --}}
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> {{-- Mengubah gap-4 menjadi gap-6 --}}
        <div>
            ...
            <input type="date" name="deadline" id="deadline" class="block mt-1 w-full rounded-lg shadow-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 transition duration-150" value="{{ old('deadline') }}" required> {{-- Mengubah rounded-md menjadi rounded-lg, menambahkan fokus, dan transisi --}}
        </div>
        <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> {{-- Mengubah gap-4 menjadi gap-6 --}}
        <div>
            <label for="status" class="block font-semibold text-sm text-gray-700 mb-1">Status</label> {{-- Menambahkan font-semibold dan mb-1 --}}
            <select name="status" id="status" class="block mt-1 w-full rounded-lg shadow-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 transition duration-150" required> {{-- Mengubah rounded-md menjadi rounded-lg, menambahkan fokus, dan transisi --}}
        ...
        <div>
            <label for="progress" class="block font-semibold text-sm text-gray-700 mb-1">Progress: <span id="progress-value">{{ old('progress', 0) }}</span>%</label> {{-- Menambahkan font-semibold dan mb-1 --}}
            <input type="range" name="progress" id="progress" min="0" max="100" class="block mt-1 w-full h-2 rounded-full appearance-none bg-gray-200 cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-indigo-600 [&::-webkit-slider-thumb]:shadow-md [&::-moz-range-thumb]:w-4 [&::-moz-range-thumb]:h-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-indigo-600 [&::-moz-range-thumb]:shadow-md" value="{{ old('progress', 0) }}" oninput="document.getElementById('progress-value').innerText = this.value"> {{-- Styling progress bar agar terlihat lebih modern --}}
        </div>
    </div>
